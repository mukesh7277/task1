...
const FeaturedTab: React.FC<FeaturedTabProps> = () => {
  ...
  return  useObserver(() => (
    <Container style={styles.tabContent}>
      {store.newest.map((item: Place) => {
        return (
          <View key={item.id}>
        <PlaceListItem data={item} />
        <Text style={{width:"50%",marginTop:5,marginBottom:10,backgroundColor:'red',fontSize:12,textAlign:'center'}} onPress={() => store.removenew(item.id)}>remove</Text>
        </View>)
      })}
    </Container>
  ));
